 ...
 */
@Component
public class ResendMsgTask extends BaseSendMsgTask implements Runnable {
			...
			for (List<String> subUserIdList : subUserIdLists) {
				try {
					if (!CollectionUtils.isEmpty(subUserIdList)) {
						OpenimCustmsgPushResponse resp = imService.pushCustomMsgToUsers(msg.getMediaId(), subUserIdList, msg.getSummary(), getMsgStr(msg),
								objectMapper.writeValueAsString(alert), "", 0L);
						if (!resp.isSuccess()) {
							logger.error("媒体号消息推送失败,消息编号:{},错误信息:{}", msg.getWid(), objectMapper.writeValueAsString(resp));
